...
@router.get("/chatroom/{chatroom_id}", response_model=list[MessageResponse])
def read_messages_by_chatroom(chatroom_id: int, db: Session = Depends(get_db)):
    return crud.get_messages_by_chatroom(db, chatroom_id=chatroom_id)
